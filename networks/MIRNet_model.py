...
def dilated_conv(in_channles, out_channels, kernel_size, bias = False, padding = 1, stride = 1, dilation = 1):
    return nn.Conv2d(in_channles, out_channels, kernel_size,
        padding=padding, bias= bias, stride = stride, dilation = dilation)
##########################################################################
...
class DAU(nn.Module):
    def __init__(
        ...
        #act, dilated_conv(n_feat, n_feat, kernel_size, padding = 2, bias=bias, dilation=2),
...
class RRG(nn.Module):
    def __init__(self, n_feat, n_MSRB, height, width, stride, bias=False):
        ...
        #kernel_size = 3
        #dilation = 5
        #modules_body.append(dilated_conv(n_feat,n_feat,kernel_size=3,dilation=5,padding=((2*(dilation-1)*(kernel_size-1)+kernel_size)-1)//2))
        self.body = nn.Sequential(*modules_body)
...
class MIRNet(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, n_feat=32, kernel_size=3, stride=2, n_RRG=3, n_MSRB=2, height=3, width=2, bias=False):
        ...
        self.dilated_conv2 = nn.Conv2d(in_channels, n_feat, kernel_size = kernel_size, padding = 2, bias = False, dilation = 2)
        self.dilated_conv3 = nn.Conv2d(in_channels, n_feat, kernel_size = kernel_size, padding = 3, bias = False, dilation = 3)
        self.dilated_conv4 = nn.Conv2d(in_channels, n_feat, kernel_size = kernel_size, padding = 4, bias = False, dilation = 4)
        self.dilated_conv5 = nn.Conv2d(in_channels, n_feat, kernel_size = kernel_size, padding = 5, bias = False, dilation = 5)
        self.dilated_conv6 = nn.Conv2d(in_channels, n_feat, kernel_size = kernel_size, padding = 6, bias = False, dilation = 6)
        self.dilated_conv7 = nn.Conv2d(in_channels, n_feat, kernel_size = kernel_size, padding = 7, bias = False, dilation = 7)
        self.dilated_conv8 = nn.Conv2d(in_channels, n_feat, kernel_size = kernel_size, padding = 8, bias = False, dilation = 8)
    ...
    def forward(self, x):
        ...
        h = self.conv_in(x)
        dilated_conv2 = self.dilated_conv2(x)
        h = h + dilated_conv2
        
        dilated_conv3 = self.dilated_conv3(x)
        h = h + dilated_conv3
        dilated_conv4 = self.dilated_conv4(x)
        dilated_conv5 = self.dilated_conv5(x)
        dilated_conv6 = self.dilated_conv6(x)
